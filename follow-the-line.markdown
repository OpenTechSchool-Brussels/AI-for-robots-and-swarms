...
Well, our aim is to start a new genre of gaming, so no better time than now. Now that you have coded your AI, time to test it out in a better setup (its file loading at the launch of ARGoS) or even better, make it combat another AI by charging it and instanciating associated robots. My goodness, can one really do that? Yes we can, and it's covered [here](./ref_setup.html#ais).


You have know all the basic tools at hand to play this racing game. For circuits, you can have a look [here](./assets/circuits.tar.gz). Don't hesitate to create your own by drawing over the background picture with your favorite image editor and modify the size of the arena up to your liking. Create the best AI to follow the track, modify on the fly its behaviour would the line be long or very sneaky, keep in memory the places where you can go full speed and much much more to get the best time lap. Once done, compare and compete with other AI!
